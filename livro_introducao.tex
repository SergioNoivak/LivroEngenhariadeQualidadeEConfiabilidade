    ...
    \begin{tikzpicture}[remember picture, overlay]
      ...
      \node[anchor=north, text width=\paperwidth, align=center]
        at ([yshift=-1.5cm]current page.north) {%
          {\Large\sffamily\color{white} SERGIO S. NOVAK,\\ ADRIANO S. de O. BAILÃO, \\ LEANDRO M. P. SANCHES}\\[0.8cm]
...
\begin{authorcard}
\centering
{\small\bfseries\color{white} ADRIANO S. de O. BAILÃO}
...
\begin{tikzpicture}[
    ...
    pass_label/.style={font=\scriptsize\itshape, color=gray!90, fill=white, inner sep=2pt},
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        ...
        lesson_red/.style={lesson, draw=red!40, top color=red!2, bottom color=red!10},
        lesson_orange/.style={lesson, draw=orange!40, top color=orange!2, bottom color=orange!10},
        lesson_green/.style={lesson, draw=green!40, top color=green!2, bottom color=green!10},
        lesson_cyan/.style={lesson, draw=cyan!40, top color=cyan!2, bottom color=cyan!10},
...
No modelo tradicional, as organizações estruturavam suas equipes de tecnologia em dois grandes grupos distintos: Desenvolvimento (Dev) e Operações (Ops). Enquanto os desenvolvedores focavam na criação de novas funcionalidades, os administradores de sistemas (sysadmins), como integrantes da equipe de operações, eram encarregados de manter os serviços em pleno funcionamento. Na prática, a rotina desses profissionais envolvia integrar componentes de software, implantar os serviços em produção, monitorar a saúde dos servidores e das redes, além de atuar diretamente na resposta a incidentes e na execução de manutenções.

O grande desafio dessa estrutura, no entanto, reside na sua escalabilidade. À medida que o serviço cresce em complexidade, infraestrutura e base de usuários, a demanda sobre a equipe de operações aumenta na mesma proporção. Consequentemente, faz-se necessário expandir o quadro de sysadmins, o que leva os custos operacionais a crescerem de forma quase linear em relação à escala do serviço, dificultando um crescimento mais sustentável.


No modelo tradicional, as organizações estruturavam suas equipes de tecnologia
em dois grandes grupos distintos: \textbf{Desenvolvimento (Dev)} e \textbf{Operações
(Ops)}. Os administradores de sistemas (\emph{sysadmins}), integrantes da equipe de operações, eram
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}
        ...
        % 1. Sombra unificada do diagrama (Deslocada para baixo e direita)
        \fill[gray!25] (-1.4, -0.1) circle[radius=3.2cm];
        \fill[gray!25] (1.6, -0.1) circle[radius=3.2cm];

        % 2. Fundo branco (Para "limpar" a parte interna da sombra)
        \fill[white] (-1.5, 0) circle[radius=3.2cm];
        \fill[white] (1.5, 0) circle[radius=3.2cm];

        % 3. Preenchimentos com gradientes radiais
        % Círculo DevOps (Esquerda)
        \fill[top color=blue!2, bottom color=blue!15] (-1.5,0) circle[radius=3.2cm];
        
        % Círculo SRE (Direita)
        \fill[top color=green!2, bottom color=green!15] (1.5,0) circle[radius=3.2cm];

        % 4. Interseção (Área compartilhada)
        \begin{scope}
            \clip (-1.5,0) circle[radius=3.2cm];
            \fill[top color=purple!5, bottom color=purple!20] (1.5,0) circle[radius=3.2cm];
        \end{scope}

        % 5. Bordas dos círculos (Desenhadas por último para sobrepor os preenchimentos)
        \draw[line width=1.5pt, blue!60] (-1.5,0) circle[radius=3.2cm];
        \draw[line width=1.5pt, green!60] (1.5,0) circle[radius=3.2cm];

        % 6. Títulos dos Círculos
        \node[font=\large\bfseries, color=blue!80!black] at (-3.2, 2.0) {DevOps};
        \node[font=\large\bfseries, color=green!80!black] at (3.2, 2.0) {SRE};

        % 7. Textos Exclusivos - DevOps
        \node[font=\small, color=blue!80!black, align=center] at (-3.0, 0.6) {\textbf{Cultura}\\[0.5ex] colaborativa};
        \node[font=\small, color=blue!80!black, align=center] at (-3.0, -1.2) {\textbf{CI/CD}\\[0.5ex] Deploy rápido};

        % 8. Textos Exclusivos - SRE
        \node[font=\small, color=green!80!black, align=center] at (3.0, 0.6) {\textbf{SLI / SLO}\\[0.5ex] Error Budget};
        \node[font=\small, color=green!80!black, align=center] at (3.0, -1.2) {\textbf{Regra 50\%}\\[0.5ex] Toil limit};
        ...
        % 10. Legenda Inferior Destacada (Caixa flutuante)
        \node[
            draw=gray!40, fill=white, thick, 
            rounded corners=6pt, inner sep=8pt, 
            font=\footnotesize\bfseries, color=gray!70!black, 
            drop shadow={opacity=0.15, shadow xshift=0.3ex, shadow yshift=-0.3ex}
        ] at (0,-4.2) {SRE = implementação concreta de princípios DevOps};
